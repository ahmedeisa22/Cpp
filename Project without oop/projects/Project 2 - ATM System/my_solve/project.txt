...
enum enWithDrawMenueOptions {
	etwenty = 1, efifty = 2,
	eOneHundred = 3, eTwoHundred = 4,
	eFourHundred = 5, eSixHundred = 6,
	eEightHundred = 7, eOneThousand = 8,
	eExist = 9
};
struct sClient
{
...
bool performWithDrawQuick(int amount) {

	if (amount > CurrentLogin.AccountBalance) {
	...
	if (ans == 'y' || ans == 'Y') {
		DeleteCurrentClient(clients, CurrentLogin.AccountNumber);
		saveDataToFile(clients,FileName);

		CurrentLogin.AccountBalance -= amount;
		// remove from deleting
		CurrentLogin.MarkDelete = false;

		clients.push_back(CurrentLogin);
		saveDataToFile(clients, FileName);

		cout << "\n\nDone Successfully, now your Balance is : " << CurrentLogin.AccountBalance << "\n";
	}
	return true;
}
void PerfromQuickWithdrawMenueOption(enWithDrawMenueOptions withdrawOptions) {
	switch (withdrawOptions)
	{
	case enWithDrawMenueOptions::etwenty: {
		performWithDrawQuick(20);
		GoBackToMainMenue();
		break;
	}
	case enWithDrawMenueOptions::efifty: {
		performWithDrawQuick(50);
		GoBackToMainMenue();
		break;
	}
		
	case enWithDrawMenueOptions::eOneHundred: {
		performWithDrawQuick(100);
		GoBackToMainMenue();
		break;
	}
	case enWithDrawMenueOptions::eTwoHundred: {
		performWithDrawQuick(200);
		GoBackToMainMenue();
		break;
	}
	case enWithDrawMenueOptions::eFourHundred: {
		performWithDrawQuick(400);
		GoBackToMainMenue();
		break;
	}
	case enWithDrawMenueOptions::eSixHundred: {
		performWithDrawQuick(600);
		GoBackToMainMenue();
		break;
	}
	case enWithDrawMenueOptions::eEightHundred: {
		performWithDrawQuick(800);
		GoBackToMainMenue();
		break;
	}
	case enWithDrawMenueOptions::eOneThousand: {
		performWithDrawQuick(1000);
		GoBackToMainMenue();
		break;
	}
	case enWithDrawMenueOptions::eExist: {
		ATM_menu();
		break;
	}
	}
...
void QuickWithDrawMenuOption() {
	...
	cout << "***********************************************\n";
	cout << "Your Balance is: " << CurrentLogin.AccountBalance << endl;
	PerfromQuickWithdrawMenueOption((enWithDrawMenueOptions)ReadWithdrawMainMenueOption());
}
...
void ShowNormalWithDraw() {
	...
	bool is_true = true;
	do {
		cout << "\nEnter an amount muptiple of 5's ? ";
		cin >> amount;
		is_true = performWithDrawQuick(amount);
	} while (amount % 5 != 0 || !is_true);
...
void ShowDeposite() {
	...
	cout << "Ok,Your Balance is: " << CurrentLogin.AccountBalance << endl;
}
